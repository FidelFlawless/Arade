...
export default async function ProductPage({
  ...
  interface ProductReviewRowData {
    id: string;
    rating: number;
    comment: string | null;
    is_verified: boolean;
  ...
  if (product) {
    ...
    const { data: revs } = await adminSupabase
      .from("reviews")
      .select("id, rating, comment, is_verified, created_at, reviewer_name, profiles(full_name)")
      .eq("product_id", product.id)
      .eq("is_approved", true)
      .order("created_at", { ascending: false });
    approvedReviews = (revs as unknown as ProductReviewRowData[]) || [];
  ...
  return (
    <>
      ...
      <ProductReviewsSection
        initialReviews={approvedReviews as ProductReviewRow[]}
