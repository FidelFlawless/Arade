...
export default async function BeautyPage() {
  const category = await getActiveCategory("beauty");
  return <><JsonLd data={{ "@context": "https://schema.org", "@graph": [ { "@type": "CollectionPage", name: category?.name || "Beauty", description: category?.description || fallbackDescription, url: absoluteUrl("/beauty"), isPartOf: { "@type": "WebSite", name: "Arade", url: absoluteUrl("/") } }, { "@type": "BreadcrumbList", itemListElement: [ { "@type": "ListItem", position: 1, name: "Home", item: absoluteUrl("/") }, { "@type": "ListItem", position: 2, name: category?.name || "Beauty", item: absoluteUrl("/beauty") } ] } ] }} /><CategoryPage categorySlug="beauty" title="Beauty" description="Discover beauty essentials curated for your everyday routine. From makeup to fragrances, find everything you need to express your style." /></>;
}
